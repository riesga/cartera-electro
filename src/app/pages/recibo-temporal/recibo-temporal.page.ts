...
@Component({
  selector: 'app-recibo-temporal',
  templateUrl: './recibo-temporal.page.html',
  styleUrls: ['./recibo-temporal.page.scss'],
  standalone: false,
})
export class ReciboTemporalPage implements OnInit {
  result_abonos: AbonoSinConexion[] | undefined

  abono!: AbonoSinConexion ;
  userName = '';
  saldo_credito = 0;
  nombre_cliente ='';
  consignado = 0;
  ruta!: RutaSinConexion;
  ...
  ngOnInit() {
    ...
    this.activatedRoute.paramMap.subscribe(params => {
      if (params.has('id')) {
        const abonos = localStorage.getItem('abonos') || '[]';
        this.abono = JSON.parse(abonos).find(
          (x: AbonoSinConexion) => x.rec_menbrete == this.activatedRoute.snapshot.paramMap.get('id'));
        if (this.abono && this.abono.codigo_credito) {
          this.obtenerConsignado(this.abono.codigo_credito, this.abono.fecha_pago);
        }
      }
    });
    ...
    this.activatedRoute.paramMap.subscribe(params => {
      if (params.has('id')) {
        ...
        const rutas = localStorage.getItem('Rutacobros') || '[]';
        this.ruta = JSON.parse(rutas).find(
          (x: RutaSinConexion) => x.codigo === Number(this.abono.codigo_credito));          
        this.saldo_credito = this.ruta.saldo_credito - this.abono.valor - this.consignado;
        this.nombre_cliente = this.ruta.nombre_cliente;
      }
    });
  }

  obtenerConsignado(codigo: string, fecha_pago: Date) {
    const abonos = localStorage.getItem('abonos') || '[]';
    this.result_abonos = JSON.parse(abonos).filter((x: AbonoSinConexion) => x.codigo_credito == codigo && x.fecha_pago < fecha_pago);
    if (this.result_abonos) {
      this.result_abonos.forEach(element => {
                                var num1 :number = +element.valor;
                                var num2:number = +this.consignado;
                                this.consignado = (num1 + num2);      
      });
    }
  }

  reciboToString(): string {
    return `Recibo Temporal:  ${this.abono.rec_menbrete}<br>
            Nombre Cliente:  ${this.nombre_cliente }<br>            
            Fecha de pago:  ${this.datePipe.transform(this.abono.fecha_pago, 'yyyy-MM-dd')}<br>
            Valor Pagado:  ${this.abono.valor.toLocaleString('en-US', { style: 'currency', currency: 'USD' })}<br>
            Por concepto de: <br>
            Abono Credito #  ${this.abono.codigo_credito}<br>
            Saldo Credito #  ${this.saldo_credito.toLocaleString('en-US', { style: 'currency', currency: 'USD' })}<br>
            Generado por: ${this.userName}<br>
            <hr>
            <center><p>¡GRACIAS POR SU PAGO!</p><center>
            <center><p>Este es un comprobante temporal. Se generará un recibo oficial cuando se sincronice.</p><center>`;
  }
  ...
  async alertConfirmar() {
    const alert = await this.alertController.create({
      ...
      header: 'INFORMACION',
      message: ('¿Esta seguro de eliminar el recibo: ' + this.abono.rec_menbrete + '?'),
      buttons: [{
        text: 'Cancelar',
        role: 'cancel',
  ...
  delete() {
    const abonos = localStorage.getItem('abonos') || '[]';
    var obtenerabonostemporales = JSON.parse(abonos);
    const index = obtenerabonostemporales.findIndex((x: AbonoSinConexion) => x.rec_menbrete == this.abono.rec_menbrete)
    obtenerabonostemporales.splice(index, 1);
    localStorage.setItem('abonos', JSON.stringify(obtenerabonostemporales));
  }
