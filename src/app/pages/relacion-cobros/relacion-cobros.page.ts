...
@Component({
  selector: 'app-relacion-cobros',
  templateUrl: './relacion-cobros.page.html',
  styleUrls: ['./relacion-cobros.page.scss'],
  standalone: false,
})
export class RelacionCobrosPage implements OnInit {
  results: any;
  resultsCobros: any;
  searchTerm: string = '';
  ...
  emptyResults$: Observable<boolean> | undefined;

  constructor(private RelacionCobros: ClientesService,
  ...
  ngOnInit() {
    ...
    this.usuarioname = this.UserService.getUsername();
    this.results = this.RelacionCobros.searchCobrosRealizados(this.usuario);
    this.resultsCobros = this.RelacionCobros.searchTotalCobros(this.usuario);
   
    this.fechaCobros = new Date().toLocaleDateString();

    this.results.subscribe((data: any[]) => {
      this.listacobros = data;
    });
    
    this.resultsCobros.subscribe((data: any[]) => {
      this.totalCobro = data.length < 1 ? null : data[0];
    });

    // Configura el observable para verificar cuando el array está vacío
  ...
  /* printReceipt() {
    ...
    var page =
      `<h2><center>RELACION DE PAGOS GESTIONADOS</center></h2>
       <h5>POR:  ${this.usuarioname}</h5>
       <h5>TOTAL COBROS:  ${this.totalCobro.valor}</h5>
       <h5>FECHA COBROS:  ${this.fechaCobros}</h5>
       <br>
    ${this.reciboToString()}`;
      ...
      if (Capacitor.isNativePlatform()) {
        ...
        try {
          ...
          this.showSuccessWithOptions(
            'Reporte guardado',
            'La imagen del reporte se ha guardado en la galería de su dispositivo en la carpeta "Pictures/Electromuebles".',
            [
              {
                text: 'Imprimir',
                handler: async () => {
                  try {
                    ...
                    await Share.share({
                      ...
                      text: 'Resumen de pagos para imprimir',
                      files: [savedFile.uri],
        ...
        printWindow.document.write(`
          <html>
            <head>
              <title>Resumen de Pagos - Electromuebles</title>
              <style>
                body {
  ...
  reciboToString(): string {
    let header = `<table align="center" border="1" style="width: 100%; border-collapse: collapse;">
    <tr>
    ...
    this.listacobros.forEach((element: { 
      ...
      totalPagos = totalPagos + Number(element.valor);
      filas += `<tr>
            <td align="center">${element.item}</td>            
            <td align="center">${element.codigo}</td>
            <td align="center">${element.recibo}</td>
            <td align="center">${element.Hora}</td>
            <td align="right">${new Intl.NumberFormat('es-CO', { style: 'currency', currency: 'COP' }).format(Number(element.valor))}</td>
          </tr>`  
    })   
    let total = `<tr align="right"><td colspan="4"><strong>Total: </strong></td><td>${new Intl.NumberFormat('es-CO', { style: 'currency', currency: 'COP' }).format(Number(totalPagos))}</td></tr>`
    let footer = `</table><center><p>Fin de la Entrega</p><center>`
    let result = header+filas+total+footer;
    return result;
  }
  ...
  generatePrintablePage(): string {
    ...
    <h2><center>REPORTE DE PAGOS RECIBIDOS</center></h2>
    <h5>POR:  ${this.usuarioname} ::: FECHA COBROS:  ${this.fechaCobros}</h5>
    ${this.reciboToString()}`;
  }
    ...
    try {
      ...
      if (Capacitor.getPlatform() === 'android') {
        ...
        try {
          ...
          const mensaje = '✅ *REPORTE DE COBROS - ELECTROMUEBLES* ✅\n\n' + 
                          'Le compartimos el reporte de cobros recibidos.\n\n';
        ...
        try {
          ...
          const mensaje = '✅ *REPORTE DE COBROS - ELECTROMUEBLES* ✅\n\n' + 
                          'Le compartimos el reporte de pagos recibidos.';
        ...
        try {
          ...
          if (navigator.share) {
            await navigator.share({
              title: 'Reporte de Pagos',
              text: '✅ REPORTE DE COBROS - ELECTROMUEBLES ✅',
              files: [file]
            });
          } else {
      ...
      this.errorAlert('No es posible compartir el reporte en este momento.');
    } finally {
      if (loading) {
        await loading.dismiss();
      }
  ...
  private async compartirTextoRecibo() {
    try {
      ...
      this.showSuccessMessage('Información del reporte compartida');
    } catch (error) {
      console.error('Error al compartir texto:', error);
      this.errorAlert('No se pudo compartir la información del reporte.');
    }
  }
